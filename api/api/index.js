// Vercel Serverless Function 入口（CommonJS 格式，适配 Vercel 默认模式）
const fs = require('fs');
...
const STATIC_FILES = {
  '/manifest.json': { rel: '../manifest.json',    type: 'application/manifest+json; charset=utf-8' },
  '/sw.js':         { rel: '../sw.js',            type: 'application/javascript; charset=utf-8' },
  '/icons/icon-512.jpg': { rel: '../icons/icon-512.jpg', type: 'image/jpeg' },
};

// Vercel Serverless Function（Express 风格 req/res）
module.exports = async function handler(req, res) {
  ...
  if (method === 'GET' && STATIC_FILES[pathname]) {
    ...
    const file = path.join(__dirname, s.rel);
  ...
  if (pathname === '/api/sms-webhook' && method === 'POST') {
    ...
    if (!recipientPhone) return res.status(400).json({ error: '未识别到接收方手机号，请在 body 中传入 phone 字段' });
    if (!/^1[3-9]\d{9}$/.test(recipientPhone)) return res.status(400).json({ error: '手机号格式不正确' });

    const result = savePickup(parsed.data, {
      source: SOURCE_SMS, content, from, phone: recipientPhone, receivedAt,
    });
    return res.status(200).json({
      parsed: true, inserted: result.inserted, record: result.record || null, data: parsed.data,
    });
  }
  ...
  if (pathname === '/api/track-query' && method === 'POST') {
    ...
    if (!result.ok) {
      return res.status(200).json({ ok: false, mock: result.mock || false, error: result.error, waybill });
    }
    ...
    if (phone) {
      const saved = savePickup(
        {
          pickupCode: data.pickupCode, station: data.station, company: data.company,
          address: null, phone, date: null, waybill: data.waybill,
          trackState: data.state, traces: data.traces,
        },
    ...
    return res.status(200).json({
      ok: true, mock: result.mock || false, inserted, record, data,
    });
  }
